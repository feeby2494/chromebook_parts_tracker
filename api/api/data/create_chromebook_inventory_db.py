...
if __name__ == "__main__":
    # Connect to db
    try:
        connection = create_connection("./" + sys.argv[1])
    except:
        print("No argument inputted for database name")
        connection = create_connection("./test_chromebook_inventory.sqlite")
    ...
    for brand in brands_list:
        for model in chromebook_parts["brands"][brand]:
            if model == None or model == " " or model == "":
                continue
            add_model_to_models_table(model, get_brand_id(connection, brand))
            ...
            for repair in chromebook_parts["brands"][brand][model]:
                if repair == None or repair == " " or repair == "":
                    continue
                add_repair_to_repairs_table(repair, chromebook_parts["brands"][brand][model][repair]["assembly"], get_model_id(connection, model))
                ...
                for part in chromebook_parts["brands"][brand][model][repair]["parts"]:
                    if part == None or part == " " or part == "" or part == "N/A".upper():
                        continue
                    add_part_to_parts_table(part, 'none', get_model_id(connection, model), get_repair_id(connection, repair))
